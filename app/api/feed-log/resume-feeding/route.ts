...
interface ResumeBody { babyId: string; side: BreastSide }
...
async function handlePost(req: NextRequest, authContext: AuthResult) {
  ...
  try {
    ...
    console.error('Error resuming feeding session:', error);
    return NextResponse.json<ApiResponse<null>>({ success: false, error: 'Failed to resume feeding session' }, { status: 500 });
  }
}
